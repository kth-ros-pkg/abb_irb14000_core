...
class YumiGripperStateInterface {
  ...
  bool init(std::string ip = "", int port = DEFAULT_STATE_PORT, int port_cmd_ommand = DEFAULT_COMMAND_PORT) 
  {
    ...
    default_tcp_connection_command.init(ip_addr, port_cmd_ommand);
    free(ip_addr);
...
public:
  ...
  {
    nh_.param<std::string>("joint_state_topic", gripper_state_topic_, "gripper_states");
    nh_.param<std::string>("grasp_request_topic_", grasp_request_topic_, "do_grasp");
    nh_.param<std::string>("grasp_release_topic_", grasp_release_topic_, "release_grasp");
    nh_.param<double>("publish_period", js_rate_, 0.1);
    nh_.param("port_state_tream", port_state_, DEFAULT_STATE_PORT);
    nh_.param("port_cmd_ommand", port_cmd_,DEFAULT_COMMAND_PORT);
    nh_.param("grasp_force", default_effort_, 5);
    nh_.param("ip", ip_, std::string("192.168.125.1") );

    heartbeat_ = nh_.createTimer(ros::Duration(js_rate_), &YumiGrippersHW::publishState, this);
    gripper_status_publisher_ = ros::NodeHandle().advertise<sensor_msgs::JointState>(gripper_state_topic_, 10); 

    left_gripper_cmd_ = 13;
    right_gripper_cmd_ = 13;

    gripper_interface_.init(ip, port_state_, port_cmd_);
    gripper_interface_.startThreads();

    left_gripper_cmd_sub_ = nh_.subscribe("/gripper_l_pos_cmd", 100, &YumiGrippersHW::leftGripperCmdCallback, this);
    right_gripper_cmd_sub_ = nh_.subscribe("/gripper_r_pos_cmd", 100, &YumiGrippersHW::rightGripperCmdCallback, this);

    ROS_INFO_STREAM("Creating a YumiGrippers HW interface");
    hardware_interface::JointStateHandle state_handle_l("gripper_l_joint", &joint_pos_[0], &joint_vel_[0], &joint_eff_[0]);
    grippers_state_interface_.registerHandle(state_handle_l);
    hardware_interface::JointStateHandle state_handle_r("gripper_r_joint", &joint_pos_[1], &joint_vel_[1], &joint_eff_[1]);
    grippers_state_interface_.registerHandle(state_handle_r);
    registerInterface(&grippers_state_interface);

    hardware_interface::JointHandle effort_handle_l(grippers_state_interface.getHandle("gripper_l_joint"), &joint_cmd_[0]);
    grippers_effort_interface_.registerHandle(effort_handle_l);
    hardware_interface::JointHandle effort_handle_r(grippers_state_interface.getHandle("gripper_r_joint"), &joint_cmd_[1]);
    grippers_effort_interface_.registerHandle(effort_handle_r);
  ...
  void read(ros::Time time, ros::Duration period) 
  {
    ...
    float left, right;
    gripper_interface_.getCurrentJointStates(left, right);
  ...
  void write(ros::Time time, ros::Duration period)
  {
    ...
    left_gripper_cmd_ = cmd_[0];
    right_gripper_cmd_ = cmd_[1];

    // ROS_INFO_STREAM("Gripper commands (left and right):");
    // ROS_INFO_STREAM(left_gripper_cmd);
    // ROS_INFO_STREAM(right_gripper_cmd);

    gripper_interface_.setGripperEfforts((int)left_gripper_cmd_, (int)right_gripper_cmd_);
...
private:
  ...
  YumiGripperStateInterface gripper_interface_;
  ros::Subscriber left_gripper_cmd_sub_;
  ros::Subscriber right_gripper_cmd_sub_;

  float left_gripper_cmd_;
  float right_gripper_cmd_;

  std::string ip_;
  std::string gripper_state_topic_;
  std::string grasp_request_topic_;
  std::string grasp_release_topic_;
  int port_state_, port_cmd_;
  double js_rate_;
  int default_effort_;
  boost::mutex data_buffer_mutex_;
  ros::Timer heartbeat_;

  hardware_interface::JointStateInterface grippers_state_interface_;
  hardware_interface::EffortJointInterface grippers_effort_interface_;
  double joint_cmd_[2];
  double joint_pos_[2];
  double joint_vel_[2];
  double joint_eff_[2];
  ...
  void leftGripperCmdCallback(const std_msgs::Float64::ConstPtr& msg)
  {
    ...
    left_gripper_cmd_ = msg -> data;
    gripper_interface_.setGripperEfforts(left_gripper_cmd_, right_gripper_cmd);
  }
  ...
  void rightGripperCmdCallback(const std_msgs::Float64::ConstPtr& msg)
  {
    ...
    right_gripper_cmd_ = msg -> data;
    gripper_interface_.setGripperEfforts(left_gripper_cmd_, right_gripper_cmd_);
  }
...
int main( int argc, char** argv )
{
  ...
  while( !g_quit )
  {
    ...
    /* Read the state from the grippers */
    yumi_grippers -> read(now, period);
    
    /* Update the controllers */
    manager.update(now, period);

    /* Write the command to the grippers */
