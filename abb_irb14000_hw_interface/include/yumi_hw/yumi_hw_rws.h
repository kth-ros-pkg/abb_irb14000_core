...
public:
  YumiHWRapid() : YumiHW() 
  {
    ...
    sampling_rate_ = 0.1;
    exponential_smoothing_alpha_ = 0.04;
  }
...
// Init, read, and write, with FRI hooks
  ...
  void read(ros::Time time, ros::Duration period)
  {
    ...
    for (int j = 0; j < n_joints_; j++)
    {
      ...
      joint_velocity_[j] = filters::exponentialSmoothing((joint_position_[j]-joint_position_prev_[j])/period.toSec(), joint_velocity_[j], exponential_smoothing_alpha_); //exponential smoothing
